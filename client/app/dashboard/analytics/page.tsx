      ...
      <div className="max-w-sm w-full flex flex-col items-center">
        ...
        <div className="flex flex-col items-center mb-8">
          <div className="h-10 w-10 rounded-xl border border-zinc-800 flex items-center justify-center mb-4 bg-zinc-900/50">
            <BarChart2 className="h-[18px] w-[18px] text-zinc-400" />
          </div>
          <h1 className="text-xl font-semibold text-white mb-1">Continue to Analytics</h1>
          ...
          <div className="flex flex-col gap-1 w-full max-h-[300px] overflow-y-auto pr-1">
            {loadingProjects ? (
              ...
              projects.map((p: any) => (
                <button
                  key={p._id}
                  onClick={() => router.push(`/dashboard/analytics/${p._id}`)}
