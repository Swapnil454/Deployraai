...
function EnvTagWithTooltip({ isProd, isLatestProd, isLatestFrontend, isLatestBackend, createdAt, supersededAt, url }: {
  isProd: boolean;
  isLatestProd: boolean;
  isLatestFrontend?: boolean;
  isLatestBackend?: boolean;
  createdAt?: string;
  supersededAt?: string;
  url?: string;
}) {
  ...
  return (
    ...
    >
      ...
      {isProd ? (
        ...
        ) : (isLatestFrontend || isLatestBackend) ? (
          <div className="flex items-center gap-1.5 px-3 py-1 bg-[#0070F3]/20 hover:bg-[#0070F3]/30 transition-colors text-[#3291FF] rounded-full font-medium text-[13px] select-none border border-transparent">
            <div className="h-[15px] w-[15px] rounded-full bg-[#0070F3] flex items-center justify-center">
              <ArrowUp className="h-[11px] w-[11px] text-white" strokeWidth={3} />
            </div>
            Production
            <span className="ml-1 text-[10px] font-bold bg-[#0070F3]/20 px-1.5 py-0.5 rounded-sm">{isLatestFrontend ? 'F' : 'B'}</span>
          </div>
        ) : (
...
function FullstackDeploymentsContent() {
              ...
              <div className="flex flex-col border border-zinc-800 rounded-xl overflow-visible bg-[#0a0a0a]">
                {(() => {
                  return deployments.map((dep: any) => {
                    ...
                    const isLatestProd = dep.isLatestProd || false;
                      ...
                      >
                        ...
                        <div className="w-[150px] shrink-0 flex items-center">
                          <EnvTagWithTooltip
                            ...
                            isLatestFrontend={dep.isLatestFrontend}
                            isLatestBackend={dep.isLatestBackend}
                        ...
                        {/* 4. Project icon + name */}
                        <div className="flex items-center gap-2 shrink-0 w-[160px] min-w-0 group/project">
                          <span className="text-zinc-200 font-medium truncate text-[15px]">{dep.projectId?.repoName || 'unknown'}</span>
                        </div>

                        {/* 5. Commit SHA */}
                        <div className="flex items-center gap-1.5 text-zinc-400 font-mono text-[14px] shrink-0 w-[90px]">
                        ...
                        <div className="flex items-center gap-3 shrink-0 ml-auto text-zinc-400 text-[14px] whitespace-nowrap w-[150px] justify-end">
