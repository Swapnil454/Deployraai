...
import { Send, Bot, User, Loader2, Copy, Check, Lock, Unlock, ExternalLink, MoreHorizontal, ChevronDown, LifeBuoy, Paperclip, ArrowUp, FileText, CheckCircle2, Search, ShieldAlert } from "lucide-react";
...
const IndustryLoader = () => {
  const [wordIndex, setWordIndex] = useState(0);

  useEffect(() => {
    const interval = setInterval(() => {
      setWordIndex((prev) => (prev + 1) % INDUSTRY_WORDS.length);
    }, 1500);
  ...
  return (
    <div className="flex items-center gap-3 py-4 text-zinc-400">
      <div className="relative flex items-center justify-center w-5 h-5">
        <span className="absolute w-full h-full border-2 border-indigo-500/30 rounded-sm animate-[spin_3s_linear_infinite]" />
        <span className="absolute w-3 h-3 bg-indigo-500/80 blur-[2px] animate-pulse" />
        <span className="absolute w-1.5 h-1.5 bg-indigo-400 rounded-sm animate-[spin_1.5s_linear_infinite_reverse]" />
      </div>
      <div className="flex flex-col">
        <span className="text-[13px] font-medium tracking-wide text-zinc-300 transition-all duration-300 ease-in-out">
          {INDUSTRY_WORDS[wordIndex]}<span className="animate-pulse">...</span>
        </span>
...
export default function SupportChat({ caseId, initialIsAdmin }: { caseId: string, initialIsAdmin: boolean }) {
  ...
  useEffect(() => {
    if (messages.length > 1 && !selectedAgent && !selectedSection) {
      const firstUserMsg = messages.find(m => m.role === 'user');
      if (firstUserMsg && firstUserMsg.content.includes('[Context provided by UI -')) {
        const matchAgent = firstUserMsg.content.match(/Selected Agent:\s([^,]+),/);
        const matchTopic = firstUserMsg.content.match(/Selected Topic:\s([^\]]+)\]/);
        if (matchAgent) setSelectedAgent(matchAgent[1].trim());
        if (matchTopic) setSelectedSection(matchTopic[1].trim());
      }
    }
  }, [messages, selectedAgent, selectedSection]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const isFirstAITurn = caseDetails?.caseType === 'ai' && messages.length === 1;
    if (isFirstAITurn && (!selectedAgent || !selectedSection)) return;
    if ((!input.trim() && attachments.length === 0) || isLoading || isUploading || caseDetails?.status === 'closed') return;

    const tempId = Date.now().toString();
    
    let finalContent = input.trim() || "Sent attachments";
    if (isFirstAITurn) {
      finalContent = `[Context provided by UI - Selected Agent: ${selectedAgent}, Selected Topic: ${selectedSection}]\n\n${finalContent}`;
    }
  ...
  return (
    <div className="flex flex-col h-full min-h-[calc(100vh-48px)] w-full relative bg-[#000000] text-zinc-200">
      ...
      <div className="px-8 py-2.5 border-b border-white/5 flex flex-col gap-2">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div className={`px-3 py-1 rounded-full text-[12px] font-medium ${caseDetails?.status === 'closed' || caseDetails?.status === 'resolved'
      ...
      <div
        ...
        className="flex-1 overflow-y-auto relative z-10 scroll-smooth flex flex-col"
      >
        <div className="px-8 pt-6 pb-0 w-full">
          <div className="max-w-[800px] space-y-4 w-full mx-auto flex flex-col pb-0">
            <div className="space-y-6 flex-1">
              ...
              {messages.map((msg, index) => {
                ...
                const showDateSeparator = currentMsgDate !== prevMsgDate && !(index === 0 && caseDetails?.caseType === 'ai');
                ...
                return (
                  <React.Fragment key={msg._id || msg.id || index}>
                    ...
                    ) : index === 0 && caseDetails?.caseType === 'ai' ? (
                      <div className="flex flex-col items-start mt-12 mb-0 text-left w-full max-w-[800px] mx-auto">
                        {/* Blue Badge */}
                        <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full bg-[#0070F3]/10 text-[#38BDF8] text-[12px] font-medium mb-6 border border-[#0070F3]/20">
                          <div className="w-1.5 h-1.5 rounded-full bg-[#0070F3]"></div>
                          All systems normal.
                        </div>

                        <h1 className="text-[40px] leading-tight font-semibold text-white tracking-tight mb-1">Deployra Support</h1>
                        <h2 className="text-[30px] text-zinc-400 font-medium mb-12 tracking-tight">How can we help you today?</h2>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full mb-12">
                          {/* Join our community */}
                          <a href="#" className="flex flex-col text-left p-6 rounded-2xl border border-white/10 bg-[#050505] hover:bg-white/5 transition-colors group">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-white font-medium flex items-center gap-1">Join our community <ExternalLink className="w-3.5 h-3.5 opacity-50 group-hover:opacity-100 transition-opacity" /></span>
                              <div className="flex -space-x-2">
                                <div className="w-6 h-6 rounded-full bg-white/10 flex items-center justify-center border border-black"><div className="w-3 h-3 bg-white rounded-sm"></div></div>
                                <div className="w-6 h-6 rounded-full bg-[#E54D2E] flex items-center justify-center border border-black text-[10px] font-bold text-white">v0</div>
                                <div className="w-6 h-6 rounded-full bg-[#0070F3] flex items-center justify-center border border-black text-[10px] font-bold text-white">S</div>
                                <div className="w-6 h-6 rounded-full bg-[#F5A623] flex items-center justify-center border border-black text-[10px] font-bold text-black">N</div>
                              </div>
                            </div>
                            <p className="text-[14px] text-zinc-400 leading-relaxed mt-2">
                              Discuss feature requests, ask questions, get help and connect with others in the Deployra community.
                            </p>
                          </a>
                          ...
                          <a href="#" className="flex flex-col text-left p-6 rounded-2xl border border-white/10 bg-[#050505] hover:bg-white/5 transition-colors group">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-white font-medium flex items-center gap-1">Talk to our sales team <ExternalLink className="w-3.5 h-3.5 opacity-50 group-hover:opacity-100 transition-opacity" /></span>
                            ...
                            <p className="text-[14px] text-zinc-400 leading-relaxed mt-2">
                              Discover the value of Deployra for your enterprise and explore our custom plans and pricing.
                            </p>
                          </a>
                        </div>
                        ...
                        <div className="w-full flex flex-col text-left mb-4">
                          <div className="flex items-center gap-2 mb-4">
                            <div className="w-6 h-6 rounded-full bg-gradient-to-br from-[#000000] to-[#333333] border border-white/20 flex items-center justify-center shadow-lg shrink-0">
                              <div className="w-3 h-3 rounded-sm bg-white"></div>
                            </div>
                            <span className="text-white font-semibold text-sm">Deployra Agent</span>
                          </div>

                          <div className="text-[15px] font-normal leading-relaxed text-zinc-200">
                            {msg.content}
                          </div>

                          <div className="flex items-center gap-4 mt-4 w-full">
                            {(messages.length === 1 || selectedAgent === 'deployra') && (
                              <button
                                onClick={() => messages.length === 1 && setSelectedAgent('deployra')}
                                className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-full border border-white/10 ${selectedAgent === 'deployra' ? 'bg-white/5 text-white' : 'bg-transparent text-zinc-400 hover:text-white'} transition-colors text-[14px] font-medium ${messages.length > 1 ? 'cursor-default pointer-events-none' : ''}`}
                              >
                                {selectedAgent === 'deployra' ? <CheckCircle2 className="w-4 h-4 text-white" /> : <div className="w-4 h-4 rounded-full border border-zinc-500"></div>}
                                Deployra
                              </button>
                            )}
                            {(messages.length === 1 || selectedAgent === 'v0') && (
                              <button
                                onClick={() => messages.length === 1 && setSelectedAgent('v0')}
                                className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-full border border-white/10 ${selectedAgent === 'v0' ? 'bg-white/5 text-white' : 'bg-transparent text-zinc-400 hover:text-white'} transition-colors text-[14px] font-medium ${messages.length > 1 ? 'cursor-default pointer-events-none' : ''}`}
                              >
                                {selectedAgent === 'v0' ? <CheckCircle2 className="w-4 h-4 text-white" /> : <div className="w-4 h-4 rounded-full border border-zinc-500"></div>}
                                v0
                              </button>
                            )}
                          </div>
                          ...
                          {selectedAgent && (
                            <div className="w-full mt-3 animate-in fade-in slide-in-from-top-4 duration-300">
                              <p className="text-[15px] font-normal leading-relaxed text-zinc-200 mb-2">
                                Thank you. Can you also let me know which section are you inquiring about?
                              </p>

                              <div className="relative">
                                <button
                                  onClick={() => messages.length === 1 && setIsDropdownOpen(!isDropdownOpen)}
                                  className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border border-white/10 bg-[#050505] transition-colors text-left ${messages.length === 1 ? 'hover:bg-white/5 cursor-pointer' : 'cursor-default'}`}
                                >
                                  <div className="flex items-center gap-3">
                                    <Search className="w-4 h-4 text-zinc-500" />
                                    <span className={selectedSection ? "text-zinc-200" : "text-zinc-500"}>
                                ...
                                {messages.length === 1 && (
                                  <div
                                    className={`absolute bottom-full left-0 right-0 mb-2 p-1.5 bg-[#0A0A0A] border border-white/10 rounded-xl shadow-2xl z-50 flex flex-col gap-1 origin-bottom transition-all duration-300 ease-out ${isDropdownOpen
                                        ? 'opacity-100 translate-y-0 scale-y-100 pointer-events-auto'
                                        : 'opacity-0 translate-y-4 scale-y-95 pointer-events-none'
                                      }`}
                                  >
                                    {[
                                      "General inquiry like plans and billing",
                                      "About projects",
                                      "Deployments analysis",
                                      "Auto fix url and automate env injections"
                                    ].map((option, idx) => (
                                      <button
                                        key={idx}
                                        onClick={() => {
                                        ...
                                        className={`w-full text-left px-4 py-2 text-[14px] rounded-lg transition-colors flex items-center justify-between ${selectedSection === option ? 'bg-[#1A1A1A] border border-white/10 text-zinc-200 font-medium' : 'bg-white/5 border border-transparent text-zinc-400 hover:bg-white/10 hover:text-zinc-200'}`}
                                      >
                                        {option}
                                        {selectedSection === option && <CheckCircle2 className="w-4 h-4 text-zinc-300" />}
                                      </button>
                                    ))}
                                  </div>
                                )}
                        ...
                        <div className={`w-full max-w-[900px] flex flex-col gap-2 text-[15px] font-normal leading-relaxed text-zinc-200 ${isSender ? "text-right items-end" : "text-left items-start"}`}>
                          ...
                          {msg.content && msg.content !== "Sent an attachment" && (
                            index === 0 && msg.role === 'user' && msg.content.startsWith("**Problem Area**") ? (() => {
                              ...
                              (() => {
                                const isBlockedMessage = msg.content?.trim().startsWith("🚫");
                                const displayContent = isBlockedMessage ? msg.content.replace("🚫", "").trim() : msg.content;

                                return isBlockedMessage ? (
                                  <div className="flex items-start gap-3 bg-red-500/5 border border-red-500/10 rounded-lg p-4 mt-2 max-w-none text-left">
                                    <ShieldAlert className="w-5 h-5 text-red-400/90 mt-0.5 shrink-0" />
                                    <div className="flex flex-col">
                                      <span className="text-sm font-medium text-red-400/90 mb-1">Security Policy Enforcement</span>
                                      <span className="text-[13px] text-zinc-300 leading-relaxed">{displayContent}</span>
                                    </div>
                                  </div>
                                ) : (
                          ...
                          {msg.content && msg.content !== "Sent an attachment" && !(index === 0 && msg.role === 'user' && msg.content.startsWith("**Problem Area**")) && (
                            <div className={`mt-0 mb-4 flex items-center ${isSender ? 'justify-end' : 'justify-start'}`}>
                              <button
                                onClick={() => handleCopyMessage(msg.content, msg._id || msg.id || index.toString())}
                                className="p-1 rounded-md text-zinc-500 hover:text-zinc-200 hover:bg-white/5 transition-colors flex items-center gap-1.5"
                                title="Copy message"
                              >
                                {copiedMessageId === (msg._id || msg.id || index.toString()) ? (
                                  <><Check className="w-3.5 h-3.5 text-emerald-500" /><span className="text-[11px] font-medium text-emerald-500">Copied</span></>
                                ) : (
        ...
        {/* Sticky Input Area */}
        <div className="sticky bottom-0 shrink-0 bg-[#000000] pt-0 pb-6 px-8 z-20 w-full">
            ...
            {caseDetails?.status === 'closed' ? (
              ...
              >
                ...
                <textarea
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && !e.shiftKey) {
                      e.preventDefault();
                      const isSelectionIncomplete = caseDetails?.caseType === 'ai' && messages.length === 1 && (!selectedAgent || !selectedSection);
                      if (!isSelectionIncomplete && (input.trim() || attachments.length > 0) && !isLoading && !isUploading) {
                        handleSubmit(e as any);
                      }
                    }
                  }}
                  placeholder={caseDetails?.caseType === 'ai' && messages.length === 1 && (!selectedAgent || !selectedSection) ? "Please select an agent and section first..." : "Send a message..."}
                  disabled={isLoading || isUploading || (caseDetails?.caseType === 'ai' && messages.length === 1 && (!selectedAgent || !selectedSection))}
                  rows={1}
                  maxLength={4000}
                ...
                <div className="flex items-center justify-between px-3 pb-2 pt-0">
                  <input type="file" ref={fileInputRef} hidden multiple onChange={handleFileUpload} disabled={attachments.length >= 5 || isUploading || isLoading || (caseDetails?.caseType === 'ai' && messages.length === 1 && (!selectedAgent || !selectedSection))} />
                  <div className="flex items-center gap-3">
                    <button type="button" onClick={() => fileInputRef.current?.click()} className="text-zinc-500 hover:text-zinc-300 transition-colors p-1 rounded-lg hover:bg-white/5 disabled:opacity-50" disabled={attachments.length >= 5 || isUploading || isLoading || (caseDetails?.caseType === 'ai' && messages.length === 1 && (!selectedAgent || !selectedSection))}>
                  ...
                  <button
                    type="submit"
                    disabled={(!input.trim() && attachments.length === 0) || isLoading || isUploading || (caseDetails?.caseType === 'ai' && messages.length === 1 && (!selectedAgent || !selectedSection))}
