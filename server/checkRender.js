...
async function run() {
  ...
  const logResponse = await fetch('https://api.render.com/v1/services/' + serviceId + '/logs?tail=true', {
    headers: { Authorization: 'Bearer ' + token }
  });
  // Since logs are a stream, we can just read the latest chunks or try an alternative approach. 
  // Wait, the API for logs is SSE, fetch handles it differently in Node. Let's just exit for now.
